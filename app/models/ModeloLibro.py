...
class ModeloLibro():
    ...
    def listar_libros(self,db):
        try:
            ...
            for row in data:
                ...
                lib=Libro(row[0],row[1],aut,row[2],row[3],None)
                libros.append(lib)
            return libros
    ...
    def leer_libro(self,db,isbn):
        try:
            cursor = db.connection.cursor()
            sql = """SELECT isbn, titulo, anoedicion, precio, url
                    FROM libro WHERE isbn = '{0}' """.format(isbn)
            cursor.execute(sql)
            data = cursor.fetchone()
            libro = Libro(data[0],data[1],None,data[2],data[3],data[4])
            return libro
        except Exception as ex:
            raise Exception(ex)

    @classmethod
    def obtener_libros_vendidos(self,db):
        try:
            cursor = db.connection.cursor()
